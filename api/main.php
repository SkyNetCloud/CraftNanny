...
foreach ($actionArray as $action) {
    switch ($action) {
        case "update_burn_rate":
            $response['update_burn_rate'] = updateBurnRate($dbConn, $_REQUEST['user_id'], $_REQUEST['burn_rate']);
            break;
        case "get_burn_rate":
                $response['get_burn_rate'] = getBurnRate($dbConn, $_REQUEST['user_id']);
                break;            
        case "doesUserExist":
        ...
        case "remove_event":
            ...
            break;
        case "load_reactor_status":
            $response['load_reactor_status'] = getReactorStatus($dbConn, $_REQUEST['user_id']);
            break;
        default:
            // No action specified
            break;
