...
function loadRedstoneControls($dbConn, $user_id) {
    // Set the correct content-type header before any output
    header('Content-Type: application/json');
    
    // Escape user input for safety
    $user_id = dbEsc($dbConn, $user_id);

    // Prepare the query to fetch the tokens for the user
    ...
    // Initialize the array to hold control data
    $controls = array();
    
    // Fetch each row and prepare control data
